...
void goodjob(int id)
{
  ...
  while (fgets(line, 100, emp))
  {
    ...
    temp = strtok(NULL, ",");
    e[i].number_of_sales = atoi(temp);

    temp = strtok(NULL, "\n");
    strcpy(e[i].name, temp);
    i++;
  }
  e[id - 100].number_of_sales += 1;
  fclose(emp);
  emp = fopen("employees.csv", "w");
  for (i = 0; i < 3; i++)
  {
    fprintf(emp, "%d,%d,%s\n", e[i].id, e[i].number_of_sales, e[i].name);
  }
  fclose(emp);
}
...
void make_bill()
{
  ...
  if (m == 0)
  {
    ...
    if (member_id != -1)
    {
      ...
      if (flag == 1)
      {
        mem[member_id - 100].number_of_donation++;
        member_updation();
      }
      else
      {
